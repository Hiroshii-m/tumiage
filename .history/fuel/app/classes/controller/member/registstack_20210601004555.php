    ...
    public function action_index()
    {
        ...
        if(Input::method() === 'POST'){
            ...
            if($val->run()){
                ...
                $url = 'https://twitter.com/intent/tweet?text='.date('m月d日', strtotime($data['created_at'])).'%0a本日：'.$data['text_num'].'文字%0a月平均：'.round($mdata['month_sum']/$month_count, 2).'文字%0a月合計：'.$mdata['month_sum'].'文字%0a年平均：'.($ydata['year_sum']/$year_count).'文字%0a年合計：'.$ydata['year_sum'].'文字%0a%0a'.$content;
                // Twitterへ投稿
