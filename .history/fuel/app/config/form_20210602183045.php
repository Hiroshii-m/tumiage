...
return array(
	...
	'form_method'          => 'post',
	'form_template'        => "\n\t\t{open}\n\t\t<table>\n{fields}\n\t\t</table>\n\t\t{close}\n",
	'fieldset_template'    => "\n\t\t<tr><td  style='display:block;' colspan=\"2\">{open}<table>\n{fields}</table></td></tr>\n\t\t{close}\n",
	'field_template'       => "\t\t<tr>\n\t\t\t<td  style='display:block;' class=\"{error_class}\">{label}{required}</td>\n\t\t\t<td class=\"{error_class}\">{field} <span>{description}</span> {error_msg}</td>\n\t\t</tr>\n",
	...
	'tabular_row_field_template' => "\t\t\t<td  style='display:block;'>{label}{required}&nbsp;{field} {error_msg}</td>\n",
