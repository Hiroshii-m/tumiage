 ...
 * @subpackage  Auth
 */
class MyAuth_Login_Simpleauth extends \Auth_Login_Driver
{
	/**
	...
	{
		$email = trim($email) ?: trim(\Input::post(\Config::get('simpleauth.username_post_key', 'username')));
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		// email重複チェックをバリデーションクラスでやるので、削除。
